...
def start_check(file, linters, prog, errors, len)
  File.open(file).each_with_index do |line, index|
    linters.each do |lint|
      errors << ErrorHolder.new(file, lint, index, line, lint.expe, lint.actual) unless lint.check(line, index)
    end
    prog.advance(100 / (len * linters.length))
  end
end
...
def start(callback, on_linter_created, on_files_load)
  ...
  begin
    ...
    argument.files.each do |file|
      ...
      start_check(file, linters, prog, errors, argument.files.length)
    end
    puts ''
